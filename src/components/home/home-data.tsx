...
// Compute the next half-hour slot label
          ...
          >
            {courts.map((court) => (
              <motion.div
                ...
                whileHover={{ y: -6, boxShadow: '0 10px 30px -10px rgba(52,211,153,0.15)' }}
                ...
                <div>
                  <div className="flex items-center gap-1 mb-2">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`w-3.5 h-3.5 ${i < Math.round(rev.overall_rating) ? 'text-yellow-400 fill-yellow-400' : 'text-[#1E3A2B]'}`}
                      />
                    ))}
